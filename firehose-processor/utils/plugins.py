...
def event_criteria_values(an_event):
    ...
    if 'tags' in criteria_values and isinstance(an_event['tags'],list) and len(an_event['tags'])>0:
    ...
    for plugin in sorted(pluginList, key=itemgetter(2), reverse=False):
        ...
        if isinstance(plugin[1], list):
            try:
                ...
                if '*' in plugin_matching_keys or plugin_matching_keys.intersection(event_tokens):
                    send = True
            except TypeError:
